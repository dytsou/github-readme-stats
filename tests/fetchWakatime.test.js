...
describe("WakaTime fetcher", () => {
  ...
  it.each([
    ["port", "wakatime.com:8080"],
    ["path", "wakatime.com/evil"],
    ["user info", "user@wakatime.com"],
    ["trailing slash", "wakatime.com/"],
  ])(
    "should normalize %s and use only hostname (SSRF protection)",
    async (_caseName, api_domain) => {
      const username = "anuraghazra";
      const encodedUsername = encodeURIComponent(username);
      mock
        .onGet(
